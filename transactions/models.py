...
class SaleDetail(models.Model):
    ...
    def __str__(self):
        """
        Returns a string representation of the SaleDetail instance with a custom receipt format.
        """
        return (
            f"Receipt Code: {self.id:04d} | "  # Example: 0001, 0002, etc.
            f"Sale ID: {self.sale.id} | "
            f"Quantity: {self.quantity} | "
            f"Total: {self.quantity * self.sale.price}"  # Assuming `price` is an attribute of `sale`
        )
